...
# Correction : on vérifie aussi la variable RENDER pour plus de fiabilité
...
MIDDLEWARE = [
    ...
    'whitenoise.middleware.WhiteNoiseMiddleware',         # 2ème — statiques (WhiteNoise)
...
if IS_PRODUCTION and DATABASE_URL:
    ...
    # Correction cruciale pour Neon/Render
...
# On garde ton existance check mais on s'assure qu'il n'est pas vide en prod
...
CKEDITOR_5_CONFIGS = {
    'article': {
        'toolbar': {
            'items': ['heading', '|', 'bold', 'italic', 'underline', 'link', 'bulletedList', 'numberedList', 'insertImage', 'codeBlock', 'undo', 'redo'],
...
LOGGING = {
    ...
    'loggers': {
        ...
        'django': {'handlers': ['console'], 'level': 'INFO' if IS_PRODUCTION else 'DEBUG', 'propagate': False},
    },
    'root': {'handlers': ['console'], 'level': 'WARNING' if IS_PRODUCTION else 'INFO'},
}
